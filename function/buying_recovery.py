...
def buying_recovery(endDate,setRange):
    ...
    for i in range(len(stock_list)):
        ...
        selectedDate = end_month['Date'].iloc[-1]
        selectedDate = pd.to_datetime(selectedDate)
        selectedDate = selectedDate.strftime("%Y-%m-%d")
        tmp = {quote:{'Selected Date': selectedDate,'Begin Month' : bm,'%Change '+bm: float('{:.2f}'.format(percent_begin)),'End Month' : em,'%Change '+em: float('{:.2f}'.format(percent_end))}}
        result.update(tmp)
    return result